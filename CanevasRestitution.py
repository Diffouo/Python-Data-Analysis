...
def CanevasRestituion(fc, sh):
        ...
        try:
                ...
                spatialJoin_ReseauxPL("PL_KA", "Reseaux", "Reso_PL_Join")
                ...
                cursor = arcpy.da.SearchCursor(fc, fields, "Code_Transfo <> '' and Code_Poste <> ''")
                count = 0
                for row in cursor:
                        count += 1
                        if (count % p) == 0:
                            arcpy.SetProgressorLabel("Processed item number {0}".format(count))
                            arcpy.SetProgressorPosition(count)
                        #----------------------------------------------------------------------------------------------------------------------------
                        arcpy.AddMessage("Printing data at the row {0} for Poste {1}".format(count, row[1]))
                        ...
                        nbPL = countFCByTransfo("PL_KA", "Code_Transfo LIKE '"+row[1]+"%' AND Data_Creator IS NOT NULL")
                        sh.write(line, 6, nbPL)
                        
                        #----------------------------------------------------------------------------------------------------------------------------
                        #Nombre de PL en fonction de la section du cable et du reseau
                        PL3par70 = summaryOfField("Reso_PL_Join", ("Stats3par70_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE '3x70mm%'", str(row[1]))
                        #PL3par70 = countFCByTransfo("Reso_PL_Join", "Code_Transfo LIKE '"+row[1]+"%' AND Section_Cable LIKE '3x70mm%'")
                        sh.write(line, 7, PL3par70)

                        PL3par50 = summaryOfField("Reso_PL_Join", ("Stats3par50_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE '3x50mm%'", str(row[1]))
                        #PL3par50 = countFCByTransfo("Reso_PL_Join", "Code_Transfo LIKE '"+row[1]+"%' AND Section_Cable LIKE '3x50mm%'")
                        sh.write(line, 8, PL3par50)

                        PL4par25 = summaryOfField("Reso_PL_Join", ("Stats4par25_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE '4x25mm%'", str(row[1]))
                        #PL4par25 = countFCByTransfo("Reso_PL_Join", "Code_Transfo LIKE '"+row[1]+"%' AND Section_Cable LIKE '4x25mm%'")
                        sh.write(line, 9, PL4par25)

                        PL4par16 = summaryOfField("Reso_PL_Join", ("Stats4par16_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE '4x16mm%'", str(row[1]))
                        #PL4par16 = countFCByTransfo("Reso_PL_Join", "Code_Transfo LIKE '"+row[1]+"%' AND Section_Cable LIKE '4x16mm%'")
                        sh.write(line, 10, PL4par16)

                        PL2par16 = summaryOfField("Reso_PL_Join", ("Stats2par16_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE '2x16mm%'", str(row[1]))
                        #PL2par16 = countFCByTransfo("Reso_PL_Join", "Code_Transfo LIKE '"+row[1]+"%' AND Section_Cable LIKE '2x16mm%'")
                        sh.write(line, 11, PL2par16)

                        PL_ClassMono = summaryOfField("Reso_PL_Join", ("StatsClassMono_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE 'Classique Monophas%'", str(row[1]))
                        ...
                        PL_ClassTri = summaryOfField("Reso_PL_Join", ("StatsClassTri_"+str(row[1])), "Code_Transfo_1 LIKE '"+row[1]+"%' AND Section_Cable LIKE 'Classique Triphas%'", str(row[1]))
                        ...
                        PL_EnDirect = countFCByTransfo("PL_KA", "Code_Transfo LIKE '"+row[1]+"%' AND Anomallies = 'AN032'")
                        sh.write(line, 14, PL_EnDirect)
                        
                        #PL avec BCC ouverte
                        PL_OpenBCC = countFCByTransfo("PL_KA", "Code_Transfo LIKE '"+row[1]+"%' AND BCC = 'BCC sans couvercle'")
                        sh.write(line, 15, PL_OpenBCC)

                        #PL avec disjoncteur compact
                        PL_CompactDisj = countFCByTransfo("PL_KA", "Code_Transfo LIKE '"+row[1]+"%' AND Calibre_Disjoncteur = 'Compact'")
                        sh.write(line, 16, PL_CompactDisj)
                        
                        #-----------------------------------------------------------------------------------------------------------------------------
                        #Longueur des reseaux par section de cable
                        len3par70 = lenghtReseauxByTransfo("Reseaux", row[1], "3x70mm")
                        sh.write(line, 17, len3par70)
                        
                        len3par50 = lenghtReseauxByTransfo("Reseaux", row[1], "3x50mm")
                        sh.write(line, 18, len3par50)
                        
                        len4par25 = lenghtReseauxByTransfo("Reseaux", row[1], "4x25mm")
                        sh.write(line, 19, len4par25)
                        
                        len4par16 = lenghtReseauxByTransfo("Reseaux", row[1], "4x16mm")
                        sh.write(line, 20, len4par16)
                        
                        len2par16 = lenghtReseauxByTransfo("Reseaux", row[1], "2x16mm")
                        sh.write(line, 21, len2par16)
                        
                        lenClassMono = lenghtReseauxByTransfo("Reseaux", row[1], "Classique Monophas")
                        sh.write(line, 22, lenClassMono)
                        
                        lenClassTriph = lenghtReseauxByTransfo("Reseaux", row[1], "Classique Triphas")
                        sh.write(line, 23, lenClassTriph)
                        
                        #-------------------------------------------------------------------------------------------------------------------------------
                        #Nombre de Supports
                        Nbre_Support = countFCByTransfo("Support", "ID_Support LIKE '"+row[1]+"%'")
                        sh.write(line, 24, Nbre_Support)

                        #Supports cassé
                        supportCasse = countFCByTransfo("Support", "ID_Support LIKE '"+row[1]+"%' AND Etat LIKE 'Cass%'")
                        sh.write(line, 25, supportCasse)

                        SupportPourri = countFCByTransfo("Support", "ID_Support LIKE '"+row[1]+"%' AND Etat = 'Pourri'")
                        sh.write(line, 26, SupportPourri)

                        SupportPourriOP = countFCByTransfo("Support", "ID_Support LIKE '"+row[1]+"%' AND Etat = 'Pourri-Operationnel'")
                        sh.write(line, 27, SupportPourriOP)

                        SupportTombe = countFCByTransfo("Support", "ID_Support LIKE '"+row[1]+"%' AND Etat LIKE 'Tomb%'")
                        sh.write(line, 28, SupportTombe)
                        
                        #-------------------------------------------------------------------------------------------------------
                        # Eclairage_Public
                        NbreEP = countFCByTransfo("Eclairage_Public", "Code_Transfo LIKE '"+row[1]+"%'")
...
excelSheet = wb.add_sheet('Restitution_Koumassi')
