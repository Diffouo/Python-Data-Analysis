...
class ToolValidator(object):
  ...
  def updateParameters(self):
    ...
    has been changed."""
    def generateOutputName(intable):
        ...
        if name:
            outfile = u'{0}_Excel'.format(os.path.join(pth, name))
        else:
            outfile = u'PL_Abnormalities_Excel'
        ...
        sheet_perf = wb.add_sheet('PL_Anomalies')
        file = outfile
        #workbook.save(file)
        return file
    ...
    if not self.params[2].altered:
        if self.params[1]:
            if self.params[1].value:
                self.params[2].value = generateOutputName(
                    self.params[1].value)
    return
